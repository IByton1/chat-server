...
PI_HOST="server.local"                       # Hostname oder IP des Pi
...
ssh -S /tmp/ssh_socket $PI_USER@$PI_HOST "sudo apt update && sudo apt install -y git curl build-essential python3"
...
# === Schritt 5: IP vom Pi holen ===
CURRENT_IP=$(ssh -S /tmp/ssh_socket $PI_USER@$PI_HOST "hostname -I | awk '{print \$1}'")
...
# === Schritt 9: pm2 installieren & Autostart einrichten ===
...
echo "🎉 Installation abgeschlossen! Dein Server läuft nun auf: https://$CURRENT_IP:3000"
